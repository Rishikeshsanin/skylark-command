...
export function OperationsDashboard({
  ...
  const knownReceivables = Math.max(health.totalWorkOrders - health.unknownReceivableCount, 0);
  const receivableCoverage = `${formatNumber(knownReceivables)} of ${formatNumber(health.totalWorkOrders)} Work Orders have known receivable values.`;
  ...
  return (
    <div className="dashboard-stack">
      <div className="metric-grid metric-grid-six executive-metric-grid">
        ...
        <MetricCard label="Receivables" value={formatAmount(health.receivables, currency)} hint={receivableCoverage} />
