...
function buildMarkdown(brief: LeadershipBriefData, currency?: string) {
  ...
  const lines = [
    ...
    "",
    "## Commercial",
    `- Won value: ${formatAmount(p.wonValue, currency)}`,
    ...
    <div className="dashboard-stack brief-layout leadership-brief-premium">
      ...
      <Panel title="Executive Summary" description="A concise, evidence-backed snapshot for leadership review.">
        <div className="metric-grid metric-grid-four executive-metric-grid">
          <MetricCard label="Open pipeline" value={formatAmount(p.openPipelineValue, currency)} hint={valueCoverage} />
          <MetricCard label="Won value" value={formatAmount(p.wonValue, currency)} hint={`${formatNumber(p.wonDeals)} won deals`} tone="positive" />
          <MetricCard label="Active WOs" value={formatNumber(w.activeWorkOrders)} hint={`${formatNumber(w.totalWorkOrders)} total`} />
          <MetricCard label="Receivables" value={formatAmount(w.receivables, currency)} hint={`${formatNumber(w.unknownReceivableCount)} unknown values`} />
        </div>
      ...
      <div className="split-grid">
        <Panel title="Operations" description="Delivery posture from the canonical Work Order health contract.">
          <div className="summary-list">
            <div><span>Active</span><strong>{formatNumber(w.activeWorkOrders)}</strong></div>
            <div><span>Ongoing</span><strong>{formatNumber(w.ongoingWorkOrders)}</strong></div>
            <div><span>Delayed</span><strong>{formatNumber(w.delayedWorkOrders)}</strong></div>
            <div><span>Paused</span><strong>{formatNumber(w.pausedWorkOrders)}</strong></div>
            <div><span>AR priority</span><strong>{formatNumber(w.arPriorityWorkOrders)}</strong></div>
          </div>
        </Panel>
        <Panel title="Cash / Receivables" description="GST-inclusive billing and collection values remain separated exactly as defined by the source contract.">
          <div className="summary-list">
            <div><span>Receivables</span><strong>{formatAmount(w.receivables, currency)}</strong></div>
            <div><span>Billed incl. GST</span><strong>{formatAmount(w.billedValueInclGst, currency)}</strong></div>
            <div><span>Collected incl. GST</span><strong>{formatAmount(w.collectedAmountInclGst, currency)}</strong></div>
            <div><span>To be billed incl. GST</span><strong>{formatAmount(w.amountToBeBilledInclGst, currency)}</strong></div>
          </div>
        </Panel>
      </div>
