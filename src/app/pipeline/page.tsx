...
export default async function PipelinePage() {
  ...
  return <div className="page"><PageHeader eyebrow="Sales Intelligence" title="Pipeline" description={description} actions={<SourceStatus fetchedAt={data.snapshot.source.fetchedAt} boardNames={[data.snapshot.source.dealsBoardName]} recordsAnalyzed={data.snapshot.deals.length} dataMode={data.snapshot.source.dataMode === "temporal" ? "temporal" : "live"} freshnessState={data.snapshot.source.freshnessState} lastSyncSucceededAt={data.snapshot.source.lastSyncSucceededAt} />} /><PipelineDashboard metrics={data.metrics} stages={data.stages} sectors={data.sectors} risks={data.risks} largestDeals={data.largestDeals} quarters={data.quarters} currency={DEFAULT_CURRENCY_CODE} /></div>;
}
