...
export default async function OverviewPage() {
  ...
  return <div className="page"><PageHeader eyebrow="Executive Command Center" title="Overview" description={description} actions={<SourceStatus fetchedAt={data.snapshot.source.fetchedAt} boardNames={[data.snapshot.source.dealsBoardName, data.snapshot.source.workOrdersBoardName]} recordsAnalyzed={data.attention.provenance.totalRecordsAnalyzed} dataMode={data.snapshot.source.dataMode === "temporal" ? "temporal" : "live"} freshnessState={data.snapshot.source.freshnessState} lastSyncSucceededAt={data.snapshot.source.lastSyncSucceededAt} />} /><OverviewDashboard pipeline={data.pipeline} workOrders={data.workOrders} sectors={data.sectors} clients={data.clients} dataQuality={data.dataQuality} attentionFeed={data.attention} currency={data.attention.currencyCode} /></div>;
}
